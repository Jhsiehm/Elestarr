...
Retire: crumpled-paper as a page texture, aging, distressing, sepia, and campaign lines from the poster ("your eyes deserve…"). The eye is a plate, not a second mark.

Visual world (pinned): light stock `#F2F1ED`, near-black ink `#16161A`, mid ink `#5A5A62`, hairline `#D6D4CC`. One saturated accent `--verify` only on the verification mark. Mono is the structural voice. Serif appears only for the human argument lines.
